module.exports = {
  'test Main page': (browser) => {
    ...
    browser.expect.elements('.multisig-wallet-tab-bar-item').count.to.equal(3)
    browser.assert.urlEquals('http://localhost:3000/#/home')
      .saveScreenshot('./test/e2e/reports/home.png')
    browser.click(".multisig-wallet-tab-bar-item-wrap > a:nth-child(2)")
    browser.assert.urlEquals('http://localhost:3000/#/vote')
      .saveScreenshot('./test/e2e/reports/vote.png')
    browser.click(".multisig-wallet-tab-bar-item-wrap > a:nth-child(3)")
